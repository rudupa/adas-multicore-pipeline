...
VehicleStateSensor::VehicleStateSensor(const std::string& name,
                                       double fps,
...
void VehicleStateSensor::start() {
    running_ = true;
    frame_counter_ = 0;
}

void VehicleStateSensor::stop() {
    running_ = false;
}

std::shared_ptr<Frame> VehicleStateSensor::generateFrame() {
    if (!running_) return nullptr;

    // Sleep for one frame period in small increments so stop() is
    // noticed promptly (within ~10 ms).
    ...
    frame->sensor_type = SensorType::VehicleState;
